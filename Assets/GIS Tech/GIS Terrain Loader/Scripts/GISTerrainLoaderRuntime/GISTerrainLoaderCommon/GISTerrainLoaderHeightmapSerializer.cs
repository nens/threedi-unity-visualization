...
namespace GISTech.GISTerrainLoader
{
    public class GISTerrainLoaderHeightmapSerializer 
    {
        ...
        public static void Serialize(GISTerrainLoaderFileData fileData,string FileName)
        {
#if UNITY_EDITOR
            string FilePath = "Assets/GIS Tech/GIS Terrain Loader/Resources/HeightmapData/" + FileName;
            
            fileData.SaveFileData(FilePath);

            var bytes = ToBytes(fileData.floatheightData);

            using (FileStream file = new FileStream(FilePath + ".bytes", FileMode.Create, FileAccess.Write))
            {
